...
<head>
    ...
    <script type="text/javascript">
                    ...
                    if(jsonResult.resultFlg==1){
                        $(jsonResult.resultData).each(function () {
                            ...
                            if(this.orderStatus==1){
                                result+="<li class='pic_a'><a href='/order/orderDetail.htm?orderId="+this.id+"'>" +
                                    "<p class='mc'>"+ordType+"（"+this.kindName+"）<span>未消费</span></p>" +
                                    "<h3><img src='"+this.headRealPath+"' ></h3>" +
                                    "<div class='font'>" +
                                    "<ul>" +
                                    "<li class='color'>"+this.goodsName+"</li>" +
                                    ...
                                    "<li>"+ordAmount+"</li></a>" +
                                    "<li class='fu'>" +
                                    "<p class='refund' onclick='confirmOrder("+this.id+");'>确认订单</p>" +
                                    "</li>" +
                                    "</ul>" +
                                    "</div>" +
                                    "</li><div class='no'></div>";
        ...
        function confirmOrder(id) {
            location.href="/order/confirm_dd.htm?orderId="+id;
        }
        //订单操作成功刷新页面，记录状态
        function refresh() {
            if(orderStatu==null){$("#all").trigger("click");}
            if(orderStatu==1){$("#start").trigger("click");}
            if(orderStatu==2){$("#pay").trigger("click");}
            if(orderStatu==3){$("#evaluate").trigger("click");}
        }
...
<div class="content">
    ...
    <div class="list">
        <ul>
            ...
            <li><a href="javascript:;" id="start">未消费</a></li>
            <li><a href="javascript:;" id="pay">待评价</a></li>
            <li><a href="javascript:;" id="evaluate">已完成</a></li>
        </ul>
    </div>
    <!--详情开始-->
    <div class="pic">
